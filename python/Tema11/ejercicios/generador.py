...
Además a la hora de hacer la lectura se mostrará en el input el **mensaje** enviado a la función. Finalmente se devolverá el valor. Esta función tiene que devolver un número, y tiene que 
...
* Una vez la tengas creada, deberás crear una nueva función llamada generador, no recibirá ningún parámetro. Dentro leerás dos números con la función **leer_numero()**:
  ...
  * El segundo número será llamado **modo** y requerirá un número entre 1 y 3, ambos incluidos. El mensaje que mostrará será: **¿Cómo quieres redondear los números? [1]Al alza [2]A la 
baja [3]Normal: **.
...
# Devuelve un número entre ini y fin
def leer_numeros(ini,fin,mensaje):
  
    num = input(mensaje)

    while num >= ini and num <= fin:
      num = input(mensaje)
...
def generador():
  ...
  modo = leer_numeros(1,3,"¿Cómo quieres redondear los números? \n[1]Al alza \n[2]A la baja \n[3]Normal\n ")
  
  lista = [random.uniform(0.1,100.9) for i in range(numeros)]
